...
class IRefIndexAdapter:
    ...
    def __init__(self, 
        ...
        self.swissprots = list(uniprot._all_uniprots("*", True)) # wat doet hij/ wat is swissport / organisme daar in ? --> from pypath it is human 
        logger.info(len(self.swissprots))
    ...
    def irefindex_process(self):
        ...
        with open('biocypher_config.yaml', 'r') as f:
    ...
    def get_nodes(self, rename_selected_fields: Union[None, list[str]] = None):
        ...
        logger.info("FINAL DATAFRAME:\n{}".format(self.final_irefindex_ints)) # --> processed dataframe
        ...
        self.nodes = []
        # Dictionary to store pubmed_ids for each node_id
        pubmed_ids_by_node = {} 
        ...
        if IRefIndexNodeType.PROTEIN in self.node_types:
            ...
            for node_id in self.nodes_ids:
                # Filter the DataFrame to get rows where the current node_id appears in either "uniprot_a" or "uniprot_b"
                node_rows = self.final_irefindex_ints[(self.final_irefindex_ints["uniprot_a"] == node_id) | (self.final_irefindex_ints["uniprot_b"] == node_id)]
                
                # Extract pubmed_ids from the filtered rows and convert them to a list
                pubmed_ids = node_rows["pubmed_ids"].tolist()
                
                # Store the list of pubmed_ids for the current node_id in the dictionary
                pubmed_ids_by_node[node_id] = pubmed_ids
                
                # Create Protein object for the current node_id with pubmed_ids
                self.nodes.append(Protein(node_id=node_id, pmid=pubmed_ids_by_node[node_id], fields=self.node_fields))
